...
- **Suspect a stale server before debugging the code.** When the UI shows old data, missing fields, or "the change didn't apply," check whether a long-running dev server is serving pre-change code *before* digging into logic. Two usual culprits: (1) the **backend** — `uvicorn instaagent_pipeline.api.app:app --port 8000` started **without `--reload`** won't pick up code changes; restart it (add `--reload`). (2) the **frontend** — a corrupted `.next` cache makes Next serve a stale build or 404 the compiled CSS (page renders unstyled); `rm -rf frontend/.next` and restart `npm run dev`. Confirm by curling the live endpoint (e.g. `/campaigns/<run_id>/scrape-stats`) and checking the JSON has the fields the current code returns.
