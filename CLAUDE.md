...
- **Supabase service role key is server-side only.** It bypasses row-level security — never put it in the dashboard/client code (`app/`, `components/`, `lib/`) or any browser code. Use it for the ingestion CLI; see `AGENTS.md` and `README.md` "Supabase Key Choice".
