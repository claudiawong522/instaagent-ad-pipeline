...
- Vector storage: Supabase `pgvector` `item_embeddings` table for ICP, format, and hook embeddings, generated by Voyage AI (`voyage-4-lite` by default).
...
5. Embeddings
   - Generate three independent embedding spaces per item:
     - `icp` (from `persona` + `target_demographic`)
     - `format` (from `content_format`, `content_category`, `visual_style`, `setting`)
     - `hook` (from `hook` verbatim)
   - Do not concatenate these into one blended embedding.
   - Implemented: `embed-items` calls Voyage AI (`POST /v1/embeddings`, batches of 128) and upserts one row per item/space/model into `item_embeddings` (pgvector, 1024 dims), idempotent via a unique constraint.
